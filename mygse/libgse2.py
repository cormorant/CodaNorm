#!/usr/bin/env python
# Python wrapper for gse_functions of Stefan Stange
# ---------------------------------------------------------------------
"""
...
def isGse2(f):
    ...
    widi = f.read(4)
    f.seek(pos)
    if widi != b'WID2': raise TypeError("File is not in GSE2 format")
...
def compress_CM6(data):
    ...
    cnt = count[0]
    if cnt < 80:
        return carr[:cnt].view(str('|S%d' % cnt))
    else:
        return carr[:(cnt // 80 + 1) * 80].view(str('|S80'))
...
def read(f, verify_chksum=True):
    ...
    if verify_chksum: verifyChecksum(f, data, version=2)
